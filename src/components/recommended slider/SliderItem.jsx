...
export default function SliderItem(props) {
  ...
  function addFavoritesHandler(){
    ...
    fetch("https://book-server.liara.run/user")
      .then((res) => res.json())
      .then((usersData) => {
        const mainUser = usersData.find((user) => user.userName === localStorage.getItem("authToken"));
        setUser(mainUser);
      })
...
// ends here
  ...
  return (
    <div className="relative w-72  text-white text-left bg-theme-400 py-5 px-2 rounded-xl">
      ...
      <Link to={`${props.id}`}>
        <h1 className=" my-3 sm:text-2xl hover:text-gray-200">
          {props.title}
        </h1>
      </Link>
      ...
      {isIntrested ? (
        <IoHeart onClick={() => removeFavoritesHandler()} className=" absolute right-2 bottom-2 text-red-400 hover:text-red-300 size-10" />
      ) : (
        <IoHeart onClick={() => addFavoritesHandler()} className=" absolute right-2 bottom-2 hover:text-red-200  size-10" />
      )}
    </div>
  );
}